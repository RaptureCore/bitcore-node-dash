...
To develop bitcore-node-rapture:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-rapture.git
...
We now will setup symlinks in `bitcore-node-rapture` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-rapture
ln -s ~/bitcoind-rpc-rapture
```
...
cd devnode
mkdir node_modules
touch bitcore-node-rapture.json
touch package.json
```

Edit `bitcore-node-rapture.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-rapture/bin/bitcore-node-rapture start
```
