...
The `bitcore-node-rapture.json` file describes which services will load for a node:
...
```json
{
  "dependencies": {
    "bitcore-lib": "^0.13.7",
    "bitcore-node-rapture": "^0.2.0",
    "insight-api": "^3.0.0"
  }
}
```

_Note:_ If you already have a bitcore-node-rapture database, and you want to query data from previous blocks in the blockchain, you will need to reindex. Reindexing right now means deleting your bitcore-node-rapture database and resyncing.
...
```js
//Require bitcore
var bitcore = require('bitcore-node-rapture');
